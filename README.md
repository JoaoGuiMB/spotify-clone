...
- Server
  - service = everything that is business rules and processors
  - controller = links the app layer and the business layer
  - routes = app layer
  - server = create the server
  - index = instance the server and expose it to the web
  - config = static data
